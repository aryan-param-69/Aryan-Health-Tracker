...
import { Activity, Droplets, Moon, Flame, Check } from "lucide-react";
...
export default function LogEntry() {
  ...
  const { data: todayLog, isLoading: isLoadingToday } = useGetTodayLog({
  ...
  React.useEffect(() => {
    if (todayLog) {
      form.reset({
        ...
        hydrationMl: todayLog.hydrationMl,
        sleepHours: todayLog.sleepHours,
        caloriesKcal: todayLog.caloriesKcal,
  ...
  if (isLoadingToday) {
    return <Skeleton className="w-full h-96 bg-white/5 rounded-2xl" />;
  }
  ...
  return (
    ...
    >
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Record today's vitality metrics.</p>
      </div>
